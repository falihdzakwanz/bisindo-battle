...
21. FEATURE ENGINEERING DARI LANDMARKS
    Raw landmarks: 21 × 3 = 63 features
...
def extract_landmarks(image_path):
    """
    Extract MediaPipe hand landmarks dari image.
    
    📚 KONSEP: Geometric Feature Extraction
    ...
    Returns:
        numpy array: (63,) flattened landmarks or None if failed
    """
    ...
    try:
        ...
        return np.array(normalized_landmarks, dtype=np.float32)
...
def process_dataset():
    ...
    print("\n" + "="*60)
    print("🤚 EXTRACTING MEDIAPIPE HAND LANDMARKS")
    print("="*60)
    ...
    for split in ["train", "val"]:
        ...
        for class_name in CLASS_NAMES:
            ...
            stats[split]["per_class"][class_name] = {
                ...
                "success_rate": class_success / len(image_files) * 100 if len(image_files) > 0 else 0
            }
            
            print(f"   ✅ Success: {class_success} | ❌ Failed: {class_failed} | Rate: {class_success/(class_success+class_failed)*100:.1f}%")
    ...
    print(f"❌ Total Failed: {total_failed}/{total_images} ({total_failed/total_images*100:.2f}%)")
